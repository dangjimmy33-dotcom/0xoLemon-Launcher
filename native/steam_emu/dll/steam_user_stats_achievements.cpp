...
Steam_User_Stats::InternalSetResult<bool> Steam_User_Stats::set_achievement_internal( const char *pchName )
{
    ...
    try {
        ...
        if (user_achievements.end() == ach || ach->value("earned", false) == false) {
            ...
            // [007Launcher] Notify the Launcher via TCP socket for real-time achievement toasts
            {
                char* portStr = nullptr;
                size_t sz = 0;
                _dupenv_s(&portStr, &sz, "ACHIEVEMENT_TCP_PORT");
                if (portStr) {
                        ...
                        if (WSAStartup(MAKEWORD(2, 2), &wsaData) == 0) {
                            ...
                            if (s != INVALID_SOCKET) {
                                ...
                                if (select(0, nullptr, &fds, nullptr, &tv) > 0) {
                                    send(s, internal_name.c_str(), (int)internal_name.size(), 0);
                                }
                                closesocket(s);
                            }
                            WSACleanup();
