    ...
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'F11') {
        ...
        import('@tauri-apps/api/window').then(async m => {
          ...
          await win.setFullscreen(!isFull).catch(console.error)
        }).catch(console.error)
      }
    }
      ...
      {isBigPictureMode ? (
        <BigPictureView
          ...
          onExit={() => {
            import('@tauri-apps/api/window').then(async m => {
              const win = m.getCurrentWindow()
              await win.setFullscreen(false).catch(console.error)
            }).catch(console.error)
