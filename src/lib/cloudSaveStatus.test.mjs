...
assert.equal(cloudSavePresentation({ state: 'synced' }).title, 'Đã đồng bộ')
assert.equal(cloudSavePresentation({ state: 'rate_limited' }).blocking, false)
assert.equal(cloudSavePresentation({ state: 'storage_full' }).title, 'Google Drive đã đầy')
...
assert.equal(cloudSavePresentation({ state: 'conflict_check_required' }).title, 'Đang kiểm tra bản Save mới')
