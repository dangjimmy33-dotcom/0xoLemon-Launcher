...
// Token for the Chat-stories media dataset (huggingface-repos.json: Chat-stories/Chat-stories)
const HF_MEDIA_TOKEN: &str = "";
...
pub fn sync_to_huggingface(app: AppHandle, game_id: String) -> Result<(), String> {
    ...
    let res = client.post(&url)
        .header("Authorization", format!("Bearer {}", HF_MEDIA_TOKEN))
        .json(&payload)
        .send()
...
/// Upload raw bytes to the HuggingFace dataset under chats/media/{filename}.
/// Returns the permanent HF resolve URL.
#[tauri::command]
pub async fn upload_chat_media(filename: String, data: Vec<u8>) -> Result<String, String> {
    // Sanitise filename: keep only safe chars to avoid path-traversal in the dataset.
    ...
    // Return the permanent public URL for the file.
...
pub async fn upload_chat_media_from_path(filename: String, filepath: String) -> Result<String, String> {
    ...
    // HuggingFace dataset files via base64 Commit API: cap at 100 MB for chat media to avoid large RAM spikes.
    let max_size: u64 = 100 * 1024 * 1024;
    if metadata.len() > max_size {
        return Err(format!("File too large. Max size is {}MB for chat media.", max_size / 1024 / 1024));
    }
...
pub async fn delete_chat_media(url: String) -> Result<(), String> {
    // Only attempt to delete files we uploaded to HF (path contains chats/media/)
    ...
    // Build an NDJSON commit that deletes the file from the dataset.
...
pub async fn get_chat_media_base64(url: String) -> Result<String, String> {
    ...
    if url.contains("huggingface.co") {
        req = req.header("Authorization", format!("Bearer {}", HF_MEDIA_TOKEN));
    }
